...
profile.HandleDefault = function()
    ...
    --[[if(gData.GetBuffCount('Haste Samba') < 1 and player.TP >= 350 and player.Status == 'Engaged')then
        AshitaCore:GetChatManager():QueueCommand(-1, '/ja "Haste Samba" <me>');
    end

    if(player.TP >= 1000 and player.Status == 'Engaged')then
        AshitaCore:GetChatManager():QueueCommand(-1, '/ws "Pyrrhic Kleos" <t>');
    end

    if((gData.GetBuffCount('Saber Dance') < 1)and player.Status == 'Engaged')then
...
profile.HandleWeaponskill = function()
    ...
    if(string.find(act.Name, 'Aeolian Edge'))then
